...
class WaitDb(path: String) extends SortedSetDb("WaitDB", path) {
  /**
    * 在等待数据库中的最大保留的时间值，以秒为单位；抓取时间超过该数值的链接
    * 将会抛弃掉
    */
  private val MaxKeepTimeSeconds = DateTime.parse("2999-01-01").getMillis / 1000
  ...
  /**
    ...
    * @param link 要压入的链接
    */
  def push(link: FetchLink): Unit = {
    val nextFetchTime: Long = System.currentTimeMillis() / 1000 +
      link.`type`.refreshInSeconds * (Math.pow(2, link.retries)).toLong

    if (nextFetchTime < 0 && nextFetchTime > MaxKeepTimeSeconds) {
      LOG.error("下次抓取时间溢出！")
    } else {
